...
const init = function() {
    ...
    player0El.classList.add('player--active'); // Klasa je već aktivna ali sam je stavio da to naglasim
...
bntRoll.addEventListener('click', function() {
    if (playing) {
        ...
        if (dice !== 1) {
            ...
            // document.getElementById(`current--${activePlayer}`).textContent = 0;
            // currentScore = 0;
            // activePlayer = activePlayer === 0 ? 1 : 0; // Different than it is
            // // toggle() : ako nema 'player--active' class dodaj je, ako ima ukloni je :) cool
            // player0El.classList.toggle('player--active');
            // player1El.classList.toggle('player--active');
